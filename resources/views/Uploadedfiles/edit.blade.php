                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group">
                                ...
                                <div class="images @error("image") is-invalid @enderror">
                                    ...
                                    @if($post->post_img)
                                        <div class="img" id="post_image" style="background-image: url({{ asset($post->post_img) }});"><span><i class="fa fa-times remove-img-icon"></i>remove</span></div>
                                    @endif
                                    <div class="pic" style="display: {{ $post->post_img ? 'none': 'flex' }}">
...
@push('scripts')
    ...
    <script>
        ...
        $('#post_image').on('click', function (){
            $(this).remove();
            $('#oldImgChanged').val('true')
            button.css('display', 'flex')
        })
        @if($cat)
            var newOption = new Option("{{ $cat->text }}", {{ $cat->id }}, true, true);
            $('.categorySelect').append(newOption).trigger('change');
        @endif

        // if the value from db is 1 will change the content to 'Active'
        if({{ $post->status }} == 1)
            $('label.custom-switch-label').attr('data-before','Active');
        // when click on the switch
